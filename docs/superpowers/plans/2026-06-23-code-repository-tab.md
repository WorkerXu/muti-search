...
- [x] **Step 1: 先写共享单元测试，锁定输入归一化、URL 生成和 partition 规则**
...
- [x] **Step 2: 实现共享模块，集中代码站点定义、仓库归一化和安全校验**
...
- [x] **Step 3: 立即补一轮共享模块自检，防止类型和冻结结构偏移**
...
- [x] **Step 1: 先给主进程补失败测试，固定代码站点白名单边界**
...
- [x] **Step 2: 在主进程中拆开搜索站点白名单和代码站点白名单，再做统一汇总**
...
- [x] **Step 3: 做一轮主进程聚焦验证，确保 attach 阻断逻辑没有回归**
...
- [x] **Step 1: 先补 renderer 失败测试，约束空态、成功导航和错误保留行为**
...
- [x] **Step 2: 在 `app.ts` 中用最小状态机替换代码占位区**
...
- [x] **Step 3: 补齐代码 Tab 样式，让工具栏、三列 pane 和状态反馈可读可测**
...
- [x] **Step 4: 做代码 Tab 集成自检，确认针对性测试外的类型约束也稳定**

Run: `npm run typecheck`

Expected: `PASS`，`app.ts` 中新增代码站点状态与 `RendererWebviewElement` 交互没有类型错误。

- [x] **Step 5: 提交 renderer 集成改动**
...
- [x] **Step 3: 跑构建，确认 Electron + Vite 打包不受新共享模块影响**
