...
默认 7 家服务全部选中。用户可以临时关闭某个服务，例如该服务未登录、卡验证码、不可用，或当前不想发送给它。
...
网格中的任一格可以临时放大查看，再返回全景。具体触发方式可以在实现阶段用鼠标双击或标题区点击确定。
...
- 本地 renderer 负责顶部控制区、响应式网格、状态条和双击放大。
...
选择 `<webview>` 的原因是第一版需要在一个本地 HTML renderer 中稳定组合顶部控件、7 个网页视图、服务状态和错误提示。后续如果需要更强的主进程视图编排能力，可以再评估 `BaseWindow + WebContentsView`。
